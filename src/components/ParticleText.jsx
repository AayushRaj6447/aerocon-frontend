  ...
  useEffect(() => {
    ...
    const startGather = (fromScatter = true) => {
      ...
      particles.forEach(particle => {
        if (fromScatter) {
          ...
          const distance = maxDim * (0.4 + particle.depth * 0.7);
          particle.x = width / 2 + Math.cos(angle) * distance + (particle.seed - 0.5) * width * 0.5;
          particle.y = height / 2 + Math.sin(angle) * distance + (particle.depth - 0.5) * height * 0.5;
        }
    ...
    const render = now => {
      ...
      if (gathering && complete) {
        gathering = false;
        if (!hasTriggeredRef.current && onCompleteRef.current) {
    ...
    const sampleText = async () => {
      ...
      // Increased particle count for bold, high-density text
      const maxParticles = Math.max(2000, Math.min(3400, Math.floor((width * height) / 90)));
      ...
      particles = selected.map((target, index) => {
        ...
        // Particles spawn across the whole viewport (not from a small box)
        const angle = seed * Math.PI * 2;
        const distance = maxDim * (0.4 + depth * 0.7);
        const startX = width / 2 + Math.cos(angle) * distance + (seed - 0.5) * width * 0.5;
        const startY = height / 2 + Math.sin(angle) * distance + (depth - 0.5) * height * 0.5;
    ...
    // Fallback timer to ensure completion triggers only after animation has fully finished
    const timer = setTimeout(() => {
      if (!hasTriggeredRef.current && onCompleteRef.current) {
        hasTriggeredRef.current = true;
        onCompleteRef.current();
      }
    }, (gatherDuration || 2800) + (stagger || 600) + 800);
    ...
    fontFamily,
    glow
    // onComplete removed from dependencies to avoid re-triggering animation!
  ]);
