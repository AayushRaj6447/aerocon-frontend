...
export default function StargazingModal({ isOpen, onClose, initialMode = 'book' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-md p-3 sm:p-6 flex items-start sm:items-center justify-center pt-8 sm:pt-6 pb-8 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-white/15 max-w-xl w-full rounded-xl sm:rounded-2xl shadow-2xl text-white relative flex flex-col max-h-[92vh] sm:max-h-[88vh] overflow-hidden my-auto">
        
        {/* Sticky/Fixed Header - Close Button NEVER gets hidden on phones or during scroll */}
        <div className="flex items-center justify-between px-5 sm:px-7 py-3.5 sm:py-4 border-b border-white/10 bg-zinc-900 shrink-0 z-20">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[11px] font-mono tracking-wider px-2 py-0.5 bg-white text-black font-semibold rounded-xs">
                Stargazing
              </span>
              <span className="text-[11px] font-mono text-zinc-400">
                Aerocon 2026 &bull; Lawn Circle
              </span>
            </div>
          ...
          {/* Touch-Friendly Close Button Permanently Anchored in Header */}
          <button
            type="button"
            onClick={onClose}
            className="p-2 -mr-1.5 rounded-full bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white transition-colors shrink-0 shadow-sm focus:outline-none"
        ...
        {/* Mode Switcher Tabs */}
        {!confirmed && (
          <div className="flex border-b border-white/10 bg-zinc-900/90 shrink-0 px-5 sm:px-7">
            <button
              type="button"
              onClick={() => {
                setActiveTab('book');
              ...
              className={`flex-1 py-2.5 text-xs font-mono font-semibold transition-colors border-b-2 -mb-[1px] ${
                activeTab === 'book'
            ...
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-mono font-semibold transition-colors border-b-2 -mb-[1px] flex items-center justify-center gap-1.5 ${
                activeTab === 'view'
                  ? 'border-white text-white bg-zinc-800/40'
                  : 'border-transparent text-zinc-400 hover:text-zinc-200'
              }`}
        ...
        <div className="overflow-y-auto px-5 sm:px-7 py-5 space-y-5 flex-1">
          {!confirmed && activeTab === 'book' ? (
            <form onSubmit={handleRegisterSubmit} className="space-y-5">
              
              {/* Meta Summary Box */}
              <div className="p-3 bg-zinc-950/80 border border-white/10 rounded-md grid grid-cols-2 gap-2 text-xs font-mono text-zinc-300">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <button
                    ...
                    className={`py-2.5 px-3.5 rounded-md text-xs font-mono border transition-all text-center flex flex-col items-center justify-center ${
                      selectedDate === '26'
                  ...
                  >
                    <span className="text-xs font-semibold">26 Sep 2026</span>
                    <span className={`text-[11px] mt-0.5 ${selectedDate === '26' ? 'text-zinc-700 font-normal' : 'text-zinc-500'}`}>
                      Day 1 (Friday)
                    </span>
                  </button>
                  <button
                  ...
                  >
                    <span className="text-xs font-semibold">27 Sep 2026</span>
                    <span className={`text-[11px] mt-0.5 ${selectedDate === '27' ? 'text-zinc-700 font-normal' : 'text-zinc-500'}`}>
                      Day 2 (Saturday)
                    </span>
                  </button>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-medium text-zinc-300 flex items-center gap-1.5">
                    ...
                    <span>2. Select Observation Time Slot ({selectedDate} Sep &bull; 7:00 PM - 8:00 PM) *</span>
                  </label>
                  <button
                    type="button"
                ...
                {loadingSlots ? (
                  <div className="p-3 bg-zinc-950 border border-white/10 rounded-md flex items-center justify-center gap-2 text-xs font-mono text-zinc-400">
                    <Loader2 className="w-4 h-4 animate-spin text-white" />
                    <span>Fetching live slots from server...</span>
                  </div>
                ) : slotsError ? (
                  <div className="p-3 bg-red-950/40 border border-red-500/40 text-red-300 text-xs font-mono rounded-md flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <button
                        type="button"
                        onClick={() => fetchSlots(true)}
                        className="mt-2 px-2.5 py-1 bg-white text-black font-semibold text-xs rounded-sm hover:bg-zinc-200"
                      >
                        Retry Connecting
                      </button>
                    </div>
                  ...
                  <div className="p-3 bg-zinc-950 border border-white/10 rounded-md text-xs font-mono text-zinc-400 text-center space-y-1">
                    <p className="text-zinc-300 font-semibold">No slots listed yet for {selectedDate} September.</p>
                    <p className="text-[11px] text-zinc-500">
                      Please select {selectedDate === '26' ? '27 Sep' : '26 Sep'} or click Refresh.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <div className="relative">
                      <select
                        ...
                        className="w-full p-2.5 sm:p-3 bg-zinc-950 border border-white/15 rounded-md text-xs sm:text-sm text-white font-mono focus:outline-none focus:border-white transition-colors cursor-pointer appearance-none pr-8"
                      >
                    ...
                    {selectedSlotId && (() => {
                      ...
                      return (
                        <div className="flex items-center justify-between px-3 py-2 bg-zinc-950 border border-white/10 rounded-md text-xs font-mono text-zinc-300">
                          <span>
                            {selectedDate} Sep &bull; Slot #{sel.slotNumber}: <strong className="text-white">{sel.displayTime}</strong>
                          </span>
                          <span className={seats <= 2 ? 'text-amber-400 font-semibold' : 'text-emerald-400 font-semibold'}>
              ...
              <div className="space-y-4 pt-1">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2.5 sm:p-3 bg-zinc-950 border border-white/15 rounded-md text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-white transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="email"
                    required
                    placeholder="e.g. btech10xxx.24@bitmesra.ac.in"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. BTECH/10XXX/24"
                      value={formData.roll}
                      onChange={(e) => setFormData({ ...formData, roll: e.target.value })}
                      className="w-full p-2.5 sm:p-3 bg-zinc-950 border border-white/15 rounded-md text-sm text-white placeholder-zinc-500 uppercase focus:outline-none focus:border-white transition-colors font-mono"
                    />
                  </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['k24', 'k25', 'k26'].map((b) => (
                        <button
                          ...
                          className={`py-2.5 px-2 rounded-md text-xs font-mono font-semibold transition-colors border ${
                            formData.batch === b
              ...
              {submitError && (
                <div className="p-3 bg-red-950/40 border border-red-500/40 text-red-300 text-xs font-mono rounded-md space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  {duplicatePass && (
                    <button
                      ...
                      className="mt-1 px-3 py-1.5 bg-white text-black font-semibold text-xs rounded-sm hover:bg-zinc-200 transition-colors flex items-center gap-1.5"
                    >
              ...
              <div className="pt-2 border-t border-white/10 flex flex-col gap-2.5">
                <button
                  ...
                  className="w-full py-3 sm:py-3.5 bg-white text-black font-semibold text-xs sm:text-sm rounded-md hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                >
                  {submitting ? (
                    <>
            ...
            <form onSubmit={handleLookupSubmit} className="space-y-4">
              <div className="p-3 bg-zinc-950/80 border border-white/10 rounded-md text-xs font-mono text-zinc-400 leading-relaxed">
                Enter your registered college email to retrieve your assigned observation slot, passcode, and download your QR boarding pass.
              </div>
              ...
              <div>
                ...
                <input
                  type="email"
                  required
                  placeholder="e.g. student@bitmesra.ac.in"
                  value={lookupEmail}
              ...
              <div className="pt-2 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-2.5 bg-white text-black font-semibold text-xs sm:text-sm rounded-md hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                >
                  {lookingUp ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
            ...
            <div className="space-y-4 animate-in zoom-in-95 duration-200">
              ...
              <div className="border border-white/20 bg-zinc-950 p-4 sm:p-5 font-mono text-xs space-y-4 rounded-lg relative">
                
                {/* Pass Header */}
                <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
                  <div>
                    <span className="text-[10px] text-zinc-500 block mb-0.5">
                      Aerocon 2026 &bull; Slot Passcode
                    </span>
                ...
                <div className="flex flex-col items-center justify-center py-2">
                  <div className="p-3 bg-white rounded-md shadow-xl inline-block">
                    <QRCodeCanvas
                ...
                {/* Attendee Metadata */}
                <div className="grid grid-cols-2 gap-2.5 border-t border-white/10 pt-3 text-[11px]">
                  <div>
                    <span className="text-zinc-500 block text-[10px]">Attendee</span>
                    <span className="text-white font-bold truncate block">{passData?.name}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 block text-[10px]">Roll & Batch</span>
                    <span className="text-white font-bold block">{passData?.roll} ({passData?.batch?.toUpperCase()})</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 block text-[10px]">Date & Slot</span>
                    <span className="text-white font-bold block">
                      {passData?.date || `${selectedDate} Sep`} &bull; Slot #{passData?.slotNumber || 1}
                    </span>
                  </div>
                  <div>
                    <span className="text-zinc-500 block text-[10px]">Time & Slot ID</span>
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-mono rounded-md transition-colors flex items-center justify-center gap-1.5 border border-white/10"
                >
                ...
                <button
                  ...
                  className="py-2.5 px-4 bg-zinc-950 hover:bg-zinc-900 text-zinc-300 text-xs font-mono rounded-md transition-colors border border-white/10"
                >
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="py-2.5 px-6 bg-white text-black font-semibold text-xs font-mono rounded-md hover:bg-zinc-200 transition-colors shadow-lg"
                >
                  Done
                </button>
              </div>
